...
void write_output_labels(AppState as,
                       ...
                       int fromRank, int toRank)
{
   ...
   // create a temporary buffer to hold the strided data to send
   float *tempBuf = new float[sendWidth * sendHeight];

   for (int i = 0; i < sendHeight; i++)
   {
      // compute the start of the source row and the dest row
      int srcRowStart = (srcOffsetRow + i) * srcWidth + srcOffsetColumn;
      // compute the start of the dest row in the temp buffer
      int destRowStart = i * sendWidth;

      // copy one row of data from the source buffer to the temp buffer
      memcpy(&tempBuf[destRowStart], &srcBuf[srcRowStart],
             sendWidth * sizeof(float));
   }

   // send the temp buffer
   MPI_Send(tempBuf, sendWidth * sendHeight, MPI_FLOAT,
            toRank, msgTag, MPI_COMM_WORLD);
                       ...
                       int fromRank, int toRank)
{
   ...
   // create a temporary buffer to hold the incoming data
   ...
   {
      int dstRowStart = (dstOffsetRow + i) * dstWidth + dstOffsetColumn;
      int srcRowStart = i * expectedWidth;
      memcpy(&dstBuf[dstRowStart], &tempBuf[srcRowStart],
                           ...
                           float *gx, float *gy)
{
   ...
   for (int di = -1; di <= 1; di++)
   {
      for (int dj = -1; dj <= 1; dj++)
      {
         ...
         {
            float pixel_val = s[ni * ncols + nj];
            Gx += pixel_val * gx[filter_idx];
            Gy += pixel_val * gy[filter_idx];
         }
